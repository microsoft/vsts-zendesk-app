        ...
        return {
            ...
            getLinkedVsoWorkItems: async function(func) {
                ...
                this.when
                    ...
                    .fail(
                        async function(jqXHR) {
                            await this.displayMain(this.getAjaxErrorMessage(jqXHR));
                        }.bind(this),
            ...
            onNewWorkItemAcceptClick: async function() {
                const _ticket3 = await wrapZafClient(this.zafClient, "ticket");
                ...
                if (this.hasFieldDefined(workItemType, "Microsoft.VSTS.Common.Severity") && $modal.find(".severity").val()) {
                    operations.push(
                        this.buildPatchToAddWorkItemField("Microsoft.VSTS.Common.Severity", $modal.find(".severity").val()),
                    );
                }
                ...
                operations.push(
                    this.buildPatchToAddWorkItemHyperlink(
                        ...
                        VSO_ZENDESK_LINK_TO_TICKET_PREFIX + _ticket3.id,
                    ),
                ...
                this.ajax("createVsoWorkItem", proj.id, workItemType.name, operations)
                    .done(
                        async function(data) {
                            ...
                            if (newWorkItemId > 0) {
                                await this.linkTicket(newWorkItemId);
                            }
                    ...
                    .fail(
                        function(jqXHR, textStatus, errorThrown) {
                            this.showErrorInModal(
                                $modal,
                                this.getAjaxErrorMessage(jqXHR, this.I18n.t("modals.link.errCannotGetWorkItem")),
                            );
                        }.bind(this),
                    );
            },
            onLinkAcceptClick: async function(event) {
                const _ticket4 = await wrapZafClient(this.zafClient, "ticket");
                ...
                var updateWorkItem = async function(workItem) {
                    ...
                    var currentLink = _.find(
                        ...
                        async function(link) {
                            if (
                                ...
                                link.attributes.name === VSO_ZENDESK_LINK_TO_TICKET_PREFIX + _ticket4.id
                            ) {
                                return link;
                            }
                        }.bind(this),
                    ...
                    if (currentLink) {
                        ...
                        var addLinkOperation = this.buildPatchToAddWorkItemHyperlink(
                            await this.buildTicketLinkUrl(),
                            VSO_ZENDESK_LINK_TO_TICKET_PREFIX + _ticket4.id,
                        );
                        this.ajax("updateVsoWorkItem", workItemId, [addLinkOperation])
                            ...
                            .fail(
                                function(jqXHR) {
                                    this.showErrorInModal(
                                        $modal,
                                        this.getAjaxErrorMessage(jqXHR, this.I18n.t("modals.link.errCannotUpdateWorkItem")),
                                    );
                                }.bind(this),
                            );
                    }
            ...
            onUnlinkAcceptClick: async function(event) {
                const _ticket5 = await wrapZafClient(this.zafClient, "ticket");
                ...
                var updateWorkItem = function(workItem) {
                    ...
                    _.each(
                        workItem.relations,
                        async function(link, idx) {
                            if (
                                link.rel.toLowerCase() === "hyperlink" &&
                                (link.attributes.name === VSO_ZENDESK_LINK_TO_TICKET_PREFIX + _ticket5.id ||
                                    link.attributes.name === VSO_ZENDESK_LINK_TO_TICKET_ATTACHMENT_PREFIX + _ticket5.id)
                            ) {
                        ...
                        this.ajax("updateVsoWorkItem", workItemId, operations)
                            ...
                            .fail(
                                function(jqXHR) {
                                    this.showErrorInModal(
                                        $modal,
                                        this.getAjaxErrorMessage(jqXHR, this.I18n.t("modals.unlink.errUnlink")),
                                    );
                                }.bind(this),
                            );
                    }
            ...
            onNotifyAcceptClick: async function() {
                const _currentUser = await wrapZafClient(this.zafClient, "currentUser");
                        ...
                        var requests = _.map(
                            ...
                            async function(workItem) {
                                ...
                                var operations = [this.buildPatchToAddWorkItemField("System.History", text)].concat(
                                    await this.buildPatchToAddWorkItemAttachments(newAttachments),
                                );
            ...
            linkTicket: async function(workItemId) {
                ...
                await wrapZafClient(this.zafClient, "ticket:tags.add", linkVsoTag);
                this.ajax("addTagToTicket", linkVsoTag);
            },
            unlinkTicket: async function(workItemId) {
                var linkVsoTag = TAG_PREFIX + workItemId;
                await wrapZafClient(this.zafClient, "ticket:tags.remove", linkVsoTag);
            ...
            getLinkedWorkItemIds: async function() {
                return _.compact(
                    await wrapZafClient(this.zafClient, "ticket:tags.map", function(t) {
            ...
            loadProjectMetadata: function(projectId) {
                ...
                var loadWorkItemTypes = this.ajax("getVsoProjectWorkItemTypes", project.id).done(
                    function(data) {
